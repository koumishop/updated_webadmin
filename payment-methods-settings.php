                        ...
                        <div class="box-body">
                            <div class="col-md-4">
                                <form method="post" id="payment_method_settings_form">
                                    ...
                                    <hr>
                                    <div class="form-group">
                                        <label for="paymobile_payment_method">Mobile Payments <small>[ Enable / Disable ] </small></label><br>
                                        <input type="checkbox" id="paymobile_payment_method_btn" class="js-switch" <?php if (isset($data['paymobile_payment_method']) && !empty($data['paymobile_payment_method']) && $data['paymobile_payment_method'] == '1') {
                                                                                                                    echo 'checked';
                                                                                                                } ?>>
                                        <input type="hidden" id="paymobile_payment_method" name="paymobile_payment_method" value="<?= (isset($data['paymobile_payment_method']) && !empty($data['paymobile_payment_method'])) ? $data['paymobile_payment_method'] : 0; ?>">
                                    </div>
                                    
                                    <h5>Carte Visa/mastercard Payments </h5>
                                    <hr>
                                    <div class="form-group">
                                        <label for="paycard_payment_method">Carte Visa/mastercard Payments <small>[ Enable / Disable ] </small></label><br>
                                        <input type="checkbox" id="paycard_payment_method_btn" class="js-switch" <?php if (isset($data['paycard_payment_method']) && !empty($data['paycard_payment_method']) && $data['paycard_payment_method'] == '1') {
                                                                                                                    echo 'checked';
                                                                                                                } ?>>
                                        <input type="hidden" id="paycard_payment_method" name="paycard_payment_method" value="<?= (isset($data['paycard_payment_method']) && !empty($data['paycard_payment_method'])) ? $data['paycard_payment_method'] : 0; ?>">
                                    </div>
...
<script type="text/javascript">
    ...
    changeCheckbox.onchange = function() {
        ...
        /* paymobile button value */
    var changeCheckbox = document.querySelector('#paymobile_payment_method_btn');
    var init = new Switchery(changeCheckbox);
    changeCheckbox.onchange = function() {
        if ($(this).is(':checked')) {
            $('#paymobile_payment_method').val(1);
        } else {
            $('#paymobile_payment_method').val(0);
        }
    };
            /* paycard button value */
    var changeCheckbox = document.querySelector('#paycard_payment_method_btn');
    var init = new Switchery(changeCheckbox);
    changeCheckbox.onchange = function() {
        if ($(this).is(':checked')) {
            $('#paycard_payment_method').val(1);
        } else {
            $('#paycard_payment_method').val(0);
        }
    };

    /* Payvisa button value */
    var changeCheckbox = document.querySelector('#payvisa_payment_method_btn');
    var init = new Switchery(changeCheckbox);
    changeCheckbox.onchange = function() {
        if ($(this).is(':checked')) {
            $('#payvisa_payment_method').val(1);
        } else {
            $('#payvisa_payment_method').val(0);
        }
    };
