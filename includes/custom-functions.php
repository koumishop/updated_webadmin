...
class custom_functions
{
    ...
    function xss_clean_array($array)
    {
     ...
     public function send_payement_notification($uid, $title, $message, $type,$messgNotification,$dateNotification)
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            //Enregistrement du notification
            $sql = "INSERT INTO notifications(`title`, `message`,`type`, `type_id`,`image`, `date_sent`,`user_id`)
                                        VALUES('$title','$messgNotification','default',0,null,'$dateNotification','$uid')";
            $this->db->sql($sql);
            //$resu = $db->getResult();
    ...
    {
        $address_data = $this->get_data($columns = ['mobile', 'latitude', 'longitude', 'address', 'pincode_id', 'city_id', 'landmark'], "id=" . $address_id, 'user_addresses');
        if ($address_data[0]['pincode_id'] == "" || $address_data[0]['city_id'] == "") {
            return false;
        }
        if (!empty($address_data)) {
            $city = $this->get_data($columns = ['name'], 'id=' . $address_data[0]['city_id'], 'cities');
            $sql = "SELECT c.*,p.pincode FROM `cities` c LEFT JOIN pincodes p on p.id=c.pincode_id where c.id= " . $address_data[0]['city_id'];
            $this->db->sql($sql);
            $res_city = $this->db->getResult();
            $user_address = $address_data[0]['address'] . "," . $address_data[0]['landmark'] . "," . $city[0]['name'] . "," . "Commune:" . $res_city[0]['pincode'];
            $order_data = array('user_address' => $user_address, 'mobile' => $address_data[0]['mobile'], 'latitude' => $address_data[0]['latitude'], 'longitude' => $address_data[0]['longitude'], 'pincode_id' => $address_data[0]['pincode_id'], 'city_id' => $address_data[0]['city_id']);
            return $order_data;
        } else {
    ...
    public function get_delivery_charge($address_id, $total = 0)
    {
        ...
        $address = $this->get_data(['city_id'], 'id=' . $address_id, 'user_addresses');
        ...
        if ((isset($config['area-wise-delivery-charge']) && !empty($config['area-wise-delivery-charge']))) {
            if (isset($address[0]['city_id']) && !empty($address[0]['city_id'])) {
                $area = $this->get_data(['delivery_charges', 'minimum_free_delivery_order_amount'], 'id=' . $address[0]['city_id'], 'cities');
