...
if (isset($_POST['change_service'])) {
    if ($permissions['services']['read'] == 1) {
        if ($_POST['service_id'] == '') {
            $sql = "SELECT * FROM services";
        } else {
            $service_id = $db->escapeString($fn->xss_clean($_POST['service_id']));
            $sql = "SELECT * FROM category WHERE service_id=" . $service_id;
        }
    } else {
    ...
    if (!empty($res)) {
        ...
        echo "<option value=''>--No Category is added--</option>";
    }
}
...
if (isset($_POST['system_configurations'])) {
    ...
    if (preg_match("/[a-z]/i", $db->escapeString($fn->xss_clean($_POST['service_charge'])))) {
        $_POST['service_charge'] = 0;
    }
    if (preg_match("/[a-z]/i", $db->escapeString($fn->xss_clean($_POST['daily_rate'])))) {
        $_POST['daily_rate'] = 0;
    }
    ...
    if ($_POST['service_charge'] <= 0) {
        echo '<label class="alert alert-danger">Service charge is not null</label>';
        return false;
    } else {
...
if (isset($_POST['add_delivery_boy']) && $_POST['add_delivery_boy'] == 1) {
    ...
    $dob = (isset($_POST['dob']) && !empty(trim($_POST['dob']))) ? $db->escapeString($fn->xss_clean($_POST['dob'])) : '';
    $bank_name = (isset($_POST['bank_name']) && !empty(trim($_POST['bank_name']))) ? $db->escapeString($fn->xss_clean($_POST['bank_name'])) : '';
    $other_payment_info = (isset($_POST['account_number']) && !empty(trim($_POST['account_number']))) ? $db->escapeString(trim($fn->xss_clean($_POST['other_payment_info']))) : '';
    $account_number = (isset($_POST['account_number']) && !empty(trim($_POST['account_number']))) ? $db->escapeString($fn->xss_clean($_POST['account_number'])) : '';
    $account_name = (isset($_POST['account_name']) && !empty(trim($_POST['account_name']))) ? $db->escapeString($fn->xss_clean($_POST['account_name'])) : '';
    $ifsc_code = (isset($_POST['ifsc_code']) && !empty(trim($_POST['ifsc_code']))) ? $db->escapeString($fn->xss_clean($_POST['ifsc_code'])) : '';
...
if (isset($_POST['update_delivery_boy']) && $_POST['update_delivery_boy'] == 1) {
    ...
    $update_bank_name =  (isset($_POST['update_bank_name']) && !empty(trim($_POST['update_bank_name']))) ? $db->escapeString($fn->xss_clean($_POST['update_bank_name'])) : '';
    $update_account_number =  (isset($_POST['update_account_number']) && !empty(trim($_POST['update_account_number']))) ? $db->escapeString($fn->xss_clean($_POST['update_account_number'])) : '';
    $update_account_name =  (isset($_POST['update_account_name']) && !empty(trim($_POST['update_account_name']))) ? $db->escapeString($fn->xss_clean($_POST['update_account_name'])) : '';
    $update_ifsc_code =  (isset($_POST['update_ifsc_code']) && !empty(trim($_POST['update_ifsc_code']))) ? $db->escapeString($fn->xss_clean($_POST['update_ifsc_code'])) : '';
...
if (isset($_POST['add_seller']) && $_POST['add_seller'] == 1) {
    ...
    $cat_id = $fn->xss_clean_array($_POST['category_ids']);
    $cat_ids = implode(",", $cat_id);
    $password = $db->escapeString($fn->xss_clean($_POST['password']));
    $password = md5($password);
    $service_id = (isset($_POST['service_id']) && $_POST['service_id'] != "") ? $db->escapeString($fn->xss_clean($_POST['service_id'])) : "0";
...
if (isset($_POST['get_subcategories_complementary']) && $_POST['get_subcategories_complementary'] != '') {

    $sql = "SELECT id, name FROM subcategory ";
    $db->sql($sql);
    $res = $db->getResult();

    // $options = '<option value="">Select Sub Categories</option>';
    foreach ($res as $option) {
        $options .= "<option value='" . $option['id'] . "'>" . $option['name'] . "</option>";
    }
