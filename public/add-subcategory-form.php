...
if (isset($_POST['btnAdd'])) {
	...
	if ($permissions['subcategories']['create'] == 1) {
		...
		$subcatcomple_id = $fn->xss_clean_array($_POST['subcategorycomplementary_ids']);
		...
		if (empty($subcatcomple_id)) {
			$error['subcategorycomplementary_ids'] = " <span class='label label-danger'>Required!</span>";
		}
		...
		if (!empty($subcategory_name) && !empty($category_subtitle) && empty($error['category_image'] && !empty($subcatcomple_ids))) {
			...
			$sql_query = "INSERT INTO subcategory (category_id, name, slug, subtitle, subcat_complementary_ids	,image)
						VALUES('$main_category', '$subcategory_name', '$slug', '$category_subtitle','$subcatcomple_ids' ,'$upload_image')";
					...
					<div class="box-body">
						...
						<div class="form-group">
							<div class="form-group">
								...
								<select multiple="multiple" size="4" name='subcategorycomplementary_ids[]' id='subcategorycomplementary_ids' class='form-control' placeholder='Enter the Subcategory IDs' required></select>
							</div>
						</div>
						<div class="form-group">
...
<script>
	$(window).on("load", function() {
		$.ajax({
			...
			data: "get_subcategories_complementary=1",
			method: "POST",
			success: function(data) {
				$('#subcategorycomplementary_ids').html("<option value=''>---Select Sub Category Complementary---</option>" + data);
			}
		});
	});
	...
	$('#subcategorycomplementary_ids').select2({
		width: 'element',
		placeholder: 'Type in Sub category name to search',
	});
</script>