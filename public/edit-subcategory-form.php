...
if (isset($_POST['btnEdit'])) {
	...
	if ($permissions['subcategories']['update'] == 1) {
		...
		$subcatcomple_id = (isset($_POST['subcategorycomplementary_ids'])) ? $fn->xss_clean_array($_POST['subcategorycomplementary_ids']) : "";
		$subcatcomple_ids = "";
		if (!empty($subcatcomple_id)) {
			$subcatcomple_ids = implode(",", $subcatcomple_id);
			$subcatcomple_ids = $db->escapeString($subcatcomple_ids);
		}
		...
		if (!empty($name) && !empty($subtitle) && empty($error['image'])) {
			if (!empty($menu_image)) {
				...
				$sql_query = "UPDATE subcategory SET category_id='" . $category . "', name = '" . $name . "', slug = '" . $slug . "',  subtitle = '" . $subtitle . "', subcat_complementary_ids	 = '" . $subcatcomple_ids . "',image = '" . $upload_image . "'
							WHERE id =" . $ID;
				$db->sql($sql_query);
				...
				$sql_query = "UPDATE subcategory SET category_id='" . $category . "', name = '" . $name . "', slug = '" . $slug . "', subtitle = '" . $subtitle . "', subcat_complementary_ids	 = '" . $subcatcomple_ids . "', image = '" . $previous_subcategory_image . "'WHERE id = '" . $ID . "'";
				$db->sql($sql_query);
			...
			<div class="box box-primary">
				...
				<form method="post" enctype="multipart/form-data" id="edit_subcategory_form">
					<div class="box-body">
						...
						<div class="form-group">
							<div class="form-group">
								<label for='subcategorycomplementary_ids'>Sub Category Complementary IDs <small>( Ex : 100,205, 360 <comma separated>)</small></label>
								<?php echo isset($error['subcategorycomplementary_ids']) ? $error['subcategorycomplementary_ids'] : ''; ?>
								<select multiple="multiple" size="4" name='subcategorycomplementary_ids[]' id='subcategorycomplementary_ids' class='form-control' placeholder='Enter the Subcategory IDs' required>
									<?php $sql = 'select id,name from `subcategory` where id != ' . $res[0]['id'] . ' order by id desc';
									$db->sql($sql);
									$result = $db->getResult();
									foreach ($result as $value) {
										$subcategories_ids = explode(',', $res[0]['subcat_complementary_ids']);
										$selected = in_array($value['id'], $subcategories_ids) ? 'selected' : '';
									?>
...
<script>
	$('#subcategorycomplementary_ids').select2({
		width: 'element',
