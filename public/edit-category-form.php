...
if (isset($_POST['btnEdit'])) {
	...
	if ($permissions['categories']['update'] == 1) {
		...
		$service_id = $db->escapeString($fn->xss_clean($_POST['services']));
		...
		if (!empty($name) && !empty($subtitle) && empty($error['image'])) {
			...
			if (!empty($menu_image)) {
				...
				$sql_query = "UPDATE category SET name = '" . $name . "', slug = '$slug', subtitle = '" . $subtitle . "', image = '" . $res[0]['image'] . "' , status = '" . $status . "', service_id = '" .$service_id."'WHERE id =" . $ID;
				$db->sql($sql_query);
				$update_result = $db->getResult();
			}
				...
				<form id="edit_category_form" method="post" enctype="multipart/form-data">
					<div class="box-body">
						<div class="form-group">
							<label for="services">Services</label><?php echo isset($error['services']) ? $error['services'] : ''; ?>
							<?php
							$db->select("services", 'id,name');
							$res_service = $db->getResult();
							?>
							<select class="form-control" id="services" name="services" >
								<?php foreach ($res_service as $row) {
									echo "<option value=" . $row['id'];
									if ($row['id'] == $res_query[0]['service_id']) {
										echo " selected";
									}
