                                                ...
                                                <div class="row col-md-12">
                                                    <div class="row">
                                                        ...
                                                        <div class="row mb-5 mt-5">
                                                            <div class="col-md-4">
                                                                <div class="delivery_rank">
                                                                    <label for="delivery_rank" class="col-form-label">Delivery rank</label>
                                                                </div>
                                                            </div>
                                                            <div class="col-md-4">
                                                                <input type="number" class="form-control" id="delivery_rank" name="delivery_rank">
                                                            </div>

                                                            <div class="col-md-4">
                                                                <div id="delivery_rank_error"></div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-4">
...
<script>
    ...
    $(document).on('click', '.update_order_item_status', function(e) {
        ...
        if(status == ""){
            alert('Désolé, il faut sélectionner le status.');
            window.location.reload();
            return false;
        }
        var status1 = status;
        // var rank = $('#delivery_rank').val();

        var id = $('#order_id').val();
        var item_id = $(this).data('value1');
        var delivery_by1 = delivery_by;
        // alert("STATUS : " + status1 + " DELIVER: " + delivery_by + " ITEM ID: " + item_id);
        // var dataStringRank = 'update_order_rank=1&order_id=' + id + '&status=' + status1 + '&order_item_id=' + item_id + '&delivery_boy_id=' + delivery_by + '&ajaxCall=1';
...
<script>
    $('.update_order_items').on('click', function(e) {
        ...
        if (confirm('Are you sure? want to update.')) {
            var dataString = 'update_order_rank=1&order_id=' + id + '&delivery_rank=' + delivery_rank + '&ajaxCall=1';
            var data = $('#update_form').serialize();
            if ($("#delivery_rank").val().length === 0 || $("#delivery_rank").val() < 1) {
                $('#delivery_rank_error').html("Merci de remplir ce champ ou la valeur saisie est incorrecte (Pas de valeur négative ou égale à zéro)");
                $('#delivery_rank_error').show().delay(3000).fadeOut();
            } else {
