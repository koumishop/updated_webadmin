...
if (isset($_POST['btnAdd'])) {
    ...
    if ($permissions['categories']['create'] == 1) {
        ...
        if (!empty($category_name) && !empty($category_subtitle) && empty($error['category_image'])) {
            ...
            $services_main = $db->escapeString($fn->xss_clean($_POST['service_name']));
            ...
            $sql_query = "INSERT INTO category (name,subtitle, image,web_image,status,slug,service_id)VALUES('$category_name', '$category_subtitle', '$upload_image','',1,'$slug','$services_main')";
            $db->sql($sql_query);
            $result = $db->getResult();
            if (!empty($result)) {
                $result = 0;
                ...
                <form method="post" enctype="multipart/form-data">
                    <div class="box-body">
                        <div class="form-group">
                            <label for="id_service_name">Service</label><?php echo isset($error['service_name']) ? $error['service_name'] : ''; ?>
                            <select class="form-control" id="id_service_name" name="service_name" required>
