...
if (isset($_POST['btnAdd'])) {
    ...
    if ($permissions['products']['create'] == 1) {
        ...
        $description = (isset($_POST['description']) && $_POST['description'] != '') ?  $db->escapeString($fn->xss_clean($_POST['description'])) : '';
        ...
        if (!empty($name) && !empty($category_id) && !empty($serve_for) && empty($error['other_images']) && empty($error['image']) && empty($error['cancelable']) && !empty($subcategory_id) && empty($error['pincode_ids_inc'])) {
            ...
            if ($product_result == 1) {
                if ($_POST['type'] == 'packet') {
                    for ($i = 0; $i < count($_POST['packate_measurement']); $i++) {
                        ...
                        $sql = "INSERT INTO product_variant (product_id,type,measurement,measurement_unit_id,price,price_original,discounted_price,serve_for,stock,stock_unit_id) VALUES('$product_id','$type','$measurement','$measurement_unit_id','$price', '$tempo_price','$discounted_price','$serve_for','$stock','$stock_unit_id')";
                        $db->sql($sql);
            ...
            <div class="box box-primary">
                ...
                <form id='add_product_form' method="post" enctype="multipart/form-data">
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            <label for="subcategory_id">Sub Category : <i class="text-danger asterik">*</i></label>
                            <?php echo isset($error['subcategory_id']) ? $error['subcategory_id'] : ''; ?>
