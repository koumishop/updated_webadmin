...
$sql_query = "SELECT image FROM promotion WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();
	...
	if ($permissions['services']['update'] == 1) {
		...
		//if (!empty($name)) {
			...
			if (!empty($menu_image)) {
				...
				//$sql_query = "UPDATE promotion SET name = ' $name',  description = '$description',image = '$upload_image', is_available='$is_available_allowed' WHERE id =  $ID";
				$sql_query = "UPDATE promotion SET description = '$description',image = '$upload_image', is_available='$is_available_allowed' WHERE id =  $ID";
				if ($db->sql($sql_query)) {
					$db->sql($sql_query);
					$update_result = $db->getResult();
				}
			} else {
				$sql_query = "UPDATE promotion SET  description = '" . $description . "', image = '" . $res[0]['image'] . "',  is_available = '" . $is_available_allowed . "' WHERE id =" . $ID;
				$db->sql($sql_query);
				$update_result = $db->getResult();
			}
			...
			// check update result
			if ($update_result == 1) {
				$error['update_services'] = " <section class='content-header'><span class='label label-success'>Promotion updated Successfully</span></section>";
			} else {
				$error['update_services'] = " <span class='label label-danger'>Failed update Promotion</span>";
			}
		}
	} else {
		$error['check_permission'] = " <section class='content-header'><span class='label label-danger'>You have no permission to update Promotion</span></section>";
	}
//}
...
$sql_query = "SELECT * FROM promotion WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();
...
<section class="content-header">
	<h1>
		Edit Promotion<small><a href='promotion.php'><i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Promotion</a></small></h1>
			...
			<div class="box box-primary">
				...
				<form id="edit_service_form" method="post" enctype="multipart/form-data">
					<div class="box-body">
						...
						<div class="form-group">
							<label for="description">Description de la promotion</label><?php echo isset($error['description']) ? $error['description'] : ''; ?>
