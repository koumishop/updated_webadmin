...
if (isset($_POST['btnEdit'])) {
	...
	if ($permissions['locations']['update'] == 1) {
		...
		$pincode_ID = $db->escapeString($fn->xss_clean($_POST['pincode_ID']));$delivery_charges = $db->escapeString($fn->xss_clean($_POST['delivery_charges']));
		...
		if (!empty($city_name) && empty($error) && !empty($pincode_ID) && !empty($delivery_charges) && !empty($minimum_free_delivery_order_amount)) {
			$sql_query = "UPDATE cities SET name = '" . $city_name . "', pincode_id = '" . $pincode_ID . "', minimum_free_delivery_order_amount = '" . $minimum_free_delivery_order_amount . "', delivery_charges = '" . $delivery_charges . "' WHERE id =" . $ID;
			$db->sql($sql_query);
				...
				<form id="edit_city_form" method="post" enctype="multipart/form-data">
					<div class="box-body">
						...
						<div class="form-group">
							...
							<input type="number" step="any" min="0" class="form-control" name="delivery_charges" id="delivery_charges" value="<?php echo $res[0]['delivery_charges']; ?>" />
						</div>
						<div class="form-group">
							...
							<input type="number" step="any" min="0" class="form-control" name="minimum_free_delivery_order_amount" id="minimum_free_delivery_order_amount" value="<?php echo $res[0]['minimum_free_delivery_order_amount']; ?>" required />
						</div>
					</div><!-- /.box-body -->
