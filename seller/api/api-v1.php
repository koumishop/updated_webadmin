...
/*
        ...
        $sql = "select oi.*,v.id as variant_id, p.name,p.image,p.manufacturer,p.made_in,p.return_status,p.cancelable_status,p.till_status,v.measurement,(select short_code from unit u where u.id=v.measurement_unit_id) as unit from order_items oi join product_variant v on oi.product_variant_id=v.id join products p on p.id=v.product_id join delivery_boys dv ON dv.id = oi.delivery_boy_id where oi.order_id=" . $row['id'] . " AND oi.seller_id=$seller_id";
        $db->sql($sql);
        */
        ...
        for ($j = 0; $j < count($res[$i]['items']); $j++) {
         ...
         $res[$i]['final_total'] = strval($final_sub_total + $res[$i]['delivery_charge'] - $res[$i]['promo_discount']);
