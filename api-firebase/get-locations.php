...
if (isset($_POST['get_pincodes']) && $_POST['get_pincodes'] == 1) {
    ...
    */
    ...
    if (!isset($_POST['city_id'])) {
        ...
        $sql = "SELECT count(p.id) as total FROM pincodes p LEFT JOIN cities c ON p.id=c.pincode_id WHERE p.status = 1 $where";
    }
    $db->sql($sql);
    $total = $db->getResult();

    $sql = "SELECT p.* FROM pincodes p LEFT JOIN cities c ON p.id=c.pincode_id where p.status = 1 $where  GROUP BY p.id ORDER BY $sort $order  LIMIT $offset,$limit";
    $db->sql($sql);
    $res = $db->getResult();
    $product = array();
    ...
    $i = 0;
    foreach ($res as $row) {
        $sql = "SELECT c.* FROM cities c JOIN pincodes p ON p.id=c.pincode_id WHERE c.pincode_id = '" . $row['id'] . "' $where ORDER BY c.id DESC";
        $db->sql($sql);
        $product[$i] = $row;
        $variants = $db->getResult();

        $product[$i]['cities'] = $variants;
        $i++;
    }
    if (!empty($product)) {
        $response['error'] = false;
...
if (isset($_POST['get_cities']) && $_POST['get_cities'] == 1) {
    ...
    */
    ...
    if (isset($_POST['pincode_id']) && !empty($_POST['pincode_id']) && is_numeric($_POST['pincode_id'])) {
        $pincode_id = (isset($_POST['pincode_id'])) ? $db->escapeString($fn->xss_clean($_POST['pincode_id'])) : "";
        $where .=  " AND p.id=" . $pincode_id;
    }
    if (isset($_POST['search']) && $_POST['search'] != '') {
        $search = $db->escapeString($fn->xss_clean($_POST['search']));
        $where .= " AND c.`id` like '%" . $search . "%' || c.`name` like '%" . $search . "%' || p.id like '%" . $search . "%' || p.name like '%" . $search . "%'";
    }

    $sql = "SELECT count(DISTINCT(c.id)) as total FROM cities c JOIN pincodes p on c.pincode_id=p.id WHERE c.status = 1 $where";
    $db->sql($sql);
    $total = $db->getResult();
    $sql = "SELECT c.*,c.name as city_name,p.*,c.id as city_id FROM `cities` c JOIN pincodes p on p.id=c.pincode_id WHERE c.status = 1  $where group by c.id";
    $db->sql($sql);
    $res = $db->getResult();
    $tempRow = $rows = array();
    foreach ($res as $row => $value) {
        $tempRow['id'] = $value['city_id'];
        $pincode_id = $value['pincode_id'];
        $sql = "SELECT p.* FROM `pincodes` p left join cities c on p.id=c.pincode_id where c.pincode_id= $pincode_id group by p.id";
        $db->sql($sql);
        $pincodes = $db->getResult();
        unset($pincodes[0]['name']);
        $tempRow['city_name'] = $value['city_name'];
        $tempRow['city_delivery_charges'] = $value['delivery_charges'];
        $tempRow['city_minimum_free_delivery_order_amount'] = $value['minimum_free_delivery_order_amount'];
        $tempRow['pincodes'] = $pincodes;
...
if (isset($_POST['get_pincodesssssssssssssssssdddddddddddddddd']) && $_POST['get_pincodesssssssssssssssssdddddddddddddddd'] == 1) {
    /*
    2. get_pincodes
        accesskey:90336
        get_pincodes:1
...
if (isset($_POST['get_citiesdddddddddddddddddddddddddsssssssss']) && $_POST['get_citiesdddddddddddddddddddddddddsssssssss'] == 1) {
    /*  
    3. get_cities
        accesskey:90336
        get_cities:1
