...
if ((isset($_POST['add_address'])) && ($_POST['add_address'] == 1)) {
    ...
    */
    ...
    if (!empty($user_id) && !empty($type) && !empty($name) && !empty($mobile) && !empty($address) && !empty($landmark) && !empty($pincode_id) && !empty($city_id)) {
        if ($is_default == 1) {
        ...
        if (!empty($res_insert)) {
            $d_charges = $fn->get_data($columns = ['minimum_free_delivery_order_amount', 'delivery_charges'], 'id=' . $city_id, 'cities');
            $res = $db->getResult();

            $sql = "select ua.*,u.name as user_name,u.mobile as mobile,p.pincode as pincode,c.name as city,c.minimum_free_delivery_order_amount as minimum_free_delivery_order_amount,c.delivery_charges as delivery_charges from user_addresses ua LEFT JOIN cities c ON c.id=ua.city_id LEFT JOIN pincodes p ON p.id=ua.pincode_id LEFT JOIN users u ON u.id=ua.user_id where ua.user_id= $user_id and ua.id=" . $res_insert[0] . " ORDER BY is_default DESC";
            $db->sql($sql);
            $res1 = $db->getResult();
...
if ((isset($_POST['update_address'])) && ($_POST['update_address'] == 1)) {
    ...
    */
    ...
    if (!empty($id) && !empty($user_id)) {
        ...
        if(empty($city_id) || empty($pincode_id)){
            $response['error'] = true;
            $response['message'] = 'Please pass city_id and pincode_id';
        }else{
            if ($fn->is_address_exists($id)) {
               ...
               /* $data = array(
                ...
                if ($db->update('user_addresses', $data, 'id=' . $id)) {
                    $d_charges = $fn->get_data($columns = ['minimum_free_delivery_order_amount', 'delivery_charges', 'name', 'pincode_id'], 'id=' . $city_id, 'cities');
                    ...
                    $response['address'] = $address;
                    $response['landmark'] = $landmark;
                    // $response['city_name'] = $d_charges[0]['name'];
                    $response['pincode_id'] = $pincode_id;
