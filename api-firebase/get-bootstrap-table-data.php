...
if (isset($_GET['table']) && $_GET['table'] == 'orders') {
    ...
    //$sql = "select o.*,u.name FROM orders o INNER JOIN users u ON u.id=o.user_id  LEFT JOIN pincodes p ON p.id=o.pincode_id LEFT JOIN order_items oi ON o.id=oi.order_id " . $where . " GROUP BY o.id ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    //$sql ="SELECT o.,u.name FROM orders o INNER JOIN users u on o.user_id=u.id "
    // echo $sql;
    $db->sql($sql);
    $res = $db->getResult();
    for ($i = 0; $i < count($res); $i++) {
        $sql = "select oi.*,d.name as d_name,s.name as s_name, u.name as uname,o.active_status as order_status from `order_items` oi 
...
// data of 'promotion' table goes here
if (isset($_GET['table']) && $_GET['table'] == 'promotion') {
    // $offset = 0;
    // $limit = 10;
    // $sort = 'id';
    // $order = 'DESC';
    ...
    // $sql = "SELECT COUNT(`id`) as total FROM `services` " . $where;
    // $db->sql($sql);
    // $res = $db->getResult();
    // foreach ($res as $row)
    //     $total = $row['total'];
    ...
    foreach ($res as $row) {
        ...
        $operate = ' <a href="edit-promotion.php?id=' . $row['id'] . '"><i class="fa fa-edit"></i>Edit</a>';
        //$operate .= ' <a class="btn-xs btn-danger" href="delete-service.php?id=' . $row['id'] . '"><i class="fa fa-trash-o"></i>Delete</a>';

        $tempRow['id'] = $row['id'];
        $tempRow['description'] = $row['description'];
        $tempRow['image'] = "<a data-lightbox='category' href='" . $row['image'] . "' data-caption='" . $row['id'] . "'><img src='" . $row['image'] . "' title='" . $row['id'] . "' height='50' /></a>";
        $tempRow['is_available'] = $row['is_available'] == 1 ? "true":"false" ;
...
// data of 'services' table goes here
if (isset($_GET['table']) && $_GET['table'] == 'services') {
    ...
    foreach ($res as $row) {
        ...
        $operate = ' <a href="edit-service.php?id=' . $row['id'] . '"><i class="fa fa-edit"></i>Edit</a>';
        $operate .= ' <a class="btn-xs btn-danger" href="delete-service.php?id=' . $row['id'] . '"><i class="fa fa-trash-o"></i>Delete</a>';
...
if (isset($_GET['table']) && $_GET['table'] == 'category') {
    ...
    $sql = "SELECT c.*,(SELECT name FROM services s WHERE s.id=c.service_id) as service_name FROM `category` c " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    $db->sql($sql);
    $res = $db->getResult();
...
if (isset($_GET['table']) && $_GET['table'] == 'subcategory') {
    ...
    foreach ($res as $row) {
        ...
        $tempRow['subtitle'] = $row['subtitle'];
        $tempRow['subcatcom'] = $row['subcat_complementary_ids'];
        $tempRow['image'] = "<a data-lightbox='category' href='" . $row['image'] . "' data-caption='" . $row['name'] . "'><img src='" . $row['image'] . "' title='" . $row['name'] . "' height='50' /></a>";
        $tempRow['operate'] = $operate;
        $rows[] = $tempRow;
    }
...
if (isset($_GET['table']) && $_GET['table'] == 'users') {
    ...
    foreach ($res as $row)
        ...
        //$sql = "SELECT u.*,a.name as area_name,p.pincode as pincode,c.name as city,ua.pincode_id FROM `users` " ;
...
if (isset($_GET['table']) && $_GET['table'] == 'seller') {
    ...
    $sql = "SELECT s.*, (SELECT name FROM services se WHERE se.id = s.service_id) as service_name FROM `seller` s $where ORDER BY $sort $order LIMIT $offset , $limit";
...
if (isset($_GET['table']) && $_GET['table'] == 'top_sellers') {
    ...
    $sql = "SELECT SUM(oi.sub_total) as total_revenue,oi.seller_id,s.name as seller_name,s.store_name,
    (SUM(oi.sub_total)-((SUM(oi.sub_total)*s.commission)/100)) as balance_seller,
    (SUM(oi.sub_total)*s.commission)/100 as commission_koumishop FROM `order_items` oi JOIN seller s on s.id=oi.seller_id where oi.date_added > DATE_SUB(NOW(), INTERVAL 1 MONTH) AND oi.active_status='delivered' GROUP BY oi.seller_id ORDER BY $sort $order LIMIT $offset, $limit";
    ...
    foreach ($res as $row) {
        ...
        $tempRow['commission_koumishop'] = number_format($row['commission_koumishop']);
        $tempRow['balance_seller'] = number_format($row['balance_seller']);
        $tempRow['operate'] = $operate;
        $i++;
        $rows[] = $tempRow;
    }
