...
if (isset($_POST['product_verification']) && $_POST['product_verification'] == 1 && isset($_POST['product_ids']) && !empty($_POST['product_ids'])) {
   /*
   7.  product_verification
       accesskey:90336     
   ...
   */
   try {
        ...
        if (empty($res)) {
            ...
            $response['message'] = "Mettez le bon parametre";
            //$response['data'] = $res;
        }
       ...
       $db->sql($sql);
       $total = $db->getResult();
        $sql = "SELECT p.id, p.name,PVariant.price,PVariant.discounted_price,PVariant.serve_for,PVariant.stock FROM `products` p LEFT JOIN `product_variant` PVariant ON PVariant.product_id=p.id WHERE  p.id IN ($product_ids)";
       $db->sql($sql);
       $res = $db->getResult();
       $product = array();
...
if (isset($_POST['get_all_products']) && $_POST['get_all_products'] == 1) {
    ...
    */
    ...
    $pincode = "";
    $service_id = (isset($_POST['service_id']) && !empty($_POST['service_id'])) ? $db->escapeString($fn->xss_clean($_POST['service_id'])) : "";
    ...
    $sql = "SELECT count(p.id) as total FROM `products` p LEFT JOIN `seller` s ON s.id=p.seller_id LEFT JOIN `category` c ON c.id=p.category_id LEFT JOIN `services` se ON se.id=c.service_id WHERE p.is_approved = 1 AND p.status = 1 AND s.status = 1 $where ";
    $db->sql($sql);
    $total = $db->getResult();

    $sql = "SELECT p.*,p.type as d_type, s.store_name as seller_name,s.slug as seller_slug,s.status as seller_status,(SELECT " . $price . " FROM product_variant pv WHERE pv.product_id=p.id) as price FROM `products` p JOIN `seller` s ON s.id=p.seller_id JOIN `category` c ON c.id=p.category_id JOIN `services` se ON se.id=c.service_id WHERE p.is_approved = 1 AND p.status = 1 AND s.status = 1  $where ORDER BY $sort $order LIMIT $offset,$limit ";
    // echo $sql;
    $db->sql($sql);
    $res = $db->getResult();
...
if (isset($_POST['get_complementary_products']) && $_POST['get_complementary_products'] == 1) {
    /*  
    ...
    */
    ...
    $sql = "SELECT subcategory_id FROM products WHERE id = $product_id";
    $db->sql($sql);
    $subcategory_id = $db->getResult();
    $subcategory_id = $subcategory_id[0]['subcategory_id'];
    if (empty($subcategory_id)) {

        $response['error'] = false;
        $response['message'] = "No complementary products found!";
        print_r(json_encode($response));
        return false;
    } else {
        $sql = "SELECT subcat_complementary_ids FROM subcategory WHERE id = $subcategory_id";
        $db->sql($sql);
        $subcat_complementary_ids = $db->getResult();

        $subcategory_ids_array = explode(",", $subcat_complementary_ids[0]['subcat_complementary_ids']);

        $total1 = 0;
        foreach ($subcategory_ids_array as $subcategory_id) {
            $subcategory_id =  intval($subcategory_id);
            $sql = "SELECT count(*) as total FROM products WHERE subcategory_id = $subcategory_id";
            $db->sql($sql);
            $total = $db->getResult();
            $total1 += $total[0]['total'];
        }
        ...
        foreach ($subcategory_ids_array as $subcategory_id) {
            ...
            $sql = "SELECT p.*,s.name as seller_name, s.id as seller_id, s.status as seller_status, pv.price FROM products p INNER JOIN seller s ON s.id=p.seller_id INNER JOIN product_variant pv ON pv.product_id = p.id WHERE subcategory_id = $subcategory_id ORDER BY $order LIMIT $offset,$limit";
            $db->sql($sql);
            $res1 = $db->getResult();
