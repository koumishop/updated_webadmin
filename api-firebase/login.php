...
if (isset($_POST['login']) && $_POST['login'] == 1) {
    ...
    */
    ...
    if (empty($_POST['password'])) {
        $response['error'] = true;
        $response['message'] = "Le mot de passe doit être rempli !";
        print_r(json_encode($response));
        return false;
    }
    ...
    if ($num == 1) {
        if ($res[0]['status'] == 0) {
            $response['error'] = true;
            $response['message'] = "Il semble que votre compte ne soit pas actif. Veuillez contacter l'administrateur pour plus d'informations.";
        } else {
            $user_id = $res[0]['id'];
            ...
            $response['error'] = true;
            $response['message'] = "Numéro ou mot de passe invalide, réessayez.";
    }
    print_r(json_encode($response));
    return false;
}
